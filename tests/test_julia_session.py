...
def _env_ready() -> bool:
    # The Manifest is generated by `Pkg.instantiate()`. Its presence is our signal that
    # the env was instantiated, the backend boots `using AgentREPL` without
    # instantiating, so without a Manifest it would fail rather than skip.
    return (
        shutil.which("julia") is not None
        and (AGENTREPL_ENV / "Project.toml").exists()
        and (AGENTREPL_ENV / "Manifest.toml").exists()
    )
...
pytestmark = [
    ...
    pytest.mark.skipif(
        not _env_ready(),
        reason=(
            "Requires Julia and an instantiated AgentREPL.jl env under "
            "src/jutul_agent/julia/agentrepl_env (run `Pkg.instantiate()` there)"
        ),
    ),
]

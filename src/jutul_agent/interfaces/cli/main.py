...
(``serve`` is a back-compatible alias for ``web``.) The active *workspace* is
``--workspace`` if given, else the current working directory. State (sessions,
traces) lives under ``--state-home`` or its default (``$XDG_DATA_HOME/jutul-agent``).
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if argv[0] in ("web", "serve"):
        from jutul_agent.interfaces.cli import serve as serve_cmd

        prog = "jutul-agent serve" if argv[0] == "serve" else "jutul-agent web"
        args = serve_cmd.build_parser(prog=prog).parse_args(argv[1:])
        return serve_cmd.run(args)  # applies the workspace flags itself

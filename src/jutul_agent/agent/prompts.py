...
def _tool_guide() -> str:
    return (
        ...
        "  - Use `julia_plot` whenever a plot would help the user. Code must "
        "return a Makie `Figure` (see the `plotting-basics` skill). Do not call "
        "`display(fig)` or open windows unless the user explicitly asks for "
